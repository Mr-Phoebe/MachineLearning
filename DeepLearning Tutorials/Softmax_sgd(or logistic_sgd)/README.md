Softmax代码详细解读
===
首先说明一点，下面的程序采用的是MSGD算法，代价函数是不带权重衰减项的，整个程序实现用Softmax回归来classfy MINST数据集（识别手写数字0～9）。  
...
代码解读见注释：  
这Logistic regression完全由权重矩阵$W$和偏置向量$b$所描述。  
分类是通过将数据投影到一系列超平面上，到超平面的距离用于确定属于该类的概率。  
```python
'''
#参数说明：
:type input: theano.tensor.TensorType
:param input: 输入的一个batch，假设一个batch有n个样本(n_example)，则input大小就是(n_example,n_in)

:type n_in: int
:param n_in: 每一个样本的大小，MNIST每个样本是一张28*28的图片，故n_in=784 

:type n_out: int
:param n_out: 输出的类别数，MNIST有0～9共10个类别，n_out=10  
''' 
class LogisticRegression(object):  
    def __init__(self, input, n_in, n_out):  
  
#W大小是n_in行n_out列，b为n_out维向量。即：每个输出对应W的一列以及b的一个元素。便于进行WX+b。    
